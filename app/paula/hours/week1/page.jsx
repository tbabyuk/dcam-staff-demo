...
export const PaulaHoursWeekOne = () => {
  ...
  const [paulaStudents, setPaulaStudents] = useState(null)
  ...
  const handleSubmitWeekOne = async (e) => {
    ...
    Object.keys(weekOneAttendance).forEach((studentName) => {
      ...
      const studentDocRef = doc(db, 'paula-students', studentName.toLowerCase());
      batch.update(studentDocRef, attendanceData);
    });

      const metaDocRef = doc(db, "meta-data", "paula")
      const metaObject = {
        payDay: closestPayday,
  ...
  const fetchData = async () => {
      ...
      // fetch Paula student info upon first render
      const studentsColRef = collection(db, "paula-students")

      const studentArray = []
      const snapshot = await getDocs(studentsColRef)
      snapshot.forEach((doc) => studentArray.push(doc.data()))
      setPaulaStudents([...studentArray])
  }
  ...
  useEffect(() => {
    ...
    if(!currentUser || currentUser.uid !== "idobqbTcReeT1Kko6gyVy03AZAm1") {
      ...
      checkWeek1AttendanceStatus("paula")
    }
  }, [authIsReady])
  ...
  useEffect(() => {
    ...
    if(paulaStudents?.length === Object.keys(weekOneAttendance).length) {
    ...
    <>
        <div className="flex flex-col w-full bg-[url('/images/main_bg.jpg')] bg-cover bg-center">
            <div className="page-header px-3 md:px-6 h-20 bg-gray-300 flex justify-between items-center col-span-2">
                <Link href="/paula"><button className="dcam-btn-rounded flex items-center"><FiArrowLeft className="inline-block me-1" />Main Page</button></Link>
                <h2 className="me-4 text-center">Your next pay day is: <br /> <span className="font-semibold">{closestPayday && closestPayday}</span></h2>
                <Link href="/paula/hours/week2"><button className="dcam-btn-rounded flex items-center">Week 2<FiArrowRight className="inline-block ms-1" /></button></Link>
            </div>
            ...
            <form className="py-10 px-3 md:px-20 lg:px-40 xl:px-56 border-r-2 border-gray-100" onSubmit={handleSubmitWeekOne}>
                ...
                <table className="bg-gray-100 w-full border-2 border-gray-200 mb-10">
                    ...
                    <tbody id="week1">
                        {paulaStudents?.map((student, index) => (
...
export default PaulaHoursWeekOne