...
export const MichelleHoursWeekOne = () => {
  ...
  useEffect(() => {
    ...
    if(!currentUser || currentUser.uid !== "2V0gFwSfbXXiATMBbtM7CeI9acI3") {
      ...
      checkWeek1AttendanceStatus("michelle")
    }
  }, [authIsReady])
  ...
  useEffect(() => {
    ...
    if(michelleStudents?.length === Object.keys(weekOneAttendance).length) {
    ...
    <>
        <div className="flex flex-col w-full bg-[url('/images/main_bg.jpg')] bg-cover bg-center">
            <div className="page-header px-3 md:px-6 h-20 bg-gray-300 flex justify-between items-center col-span-2">
                <Link href="/michelle"><button className="dcam-btn-rounded flex items-center"><FiArrowLeft className="inline-block me-1" />Main Page</button></Link>
                <h2 className="me-4 text-center">Your next pay day is: <br /> <span className="font-semibold">{closestPayday && closestPayday}</span></h2>
                <Link href="/michelle/hours/week2"><button className="dcam-btn-rounded flex items-center">Week 2<FiArrowRight className="inline-block ms-1" /></button></Link>
            </div>
...
export default MichelleHoursWeekOne