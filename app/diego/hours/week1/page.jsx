...
export const DiegoHoursWeekOne = () => {
  ...
  const [diegoStudents, setDiegoStudents] = useState(null)
  ...
  const handleSubmitWeekOne = async (e) => {
    ...
    Object.keys(weekOneAttendance).forEach((studentName) => {
      ...
      const studentDocRef = doc(db, 'diego-students', studentName.toLowerCase());
      batch.update(studentDocRef, attendanceData);
    });

      const metaDocRef = doc(db, "meta-data", "diego")
      const metaObject = {
        payDay: closestPayday,
  ...
  const fetchData = async () => {
      ...
      // fetch Diego student info upon first render
      const studentsColRef = collection(db, "diego-students")

      const studentArray = []
      const snapshot = await getDocs(studentsColRef)
      snapshot.forEach((doc) => studentArray.push(doc.data()))
      setDiegoStudents([...studentArray])
  }
  ...
  useEffect(() => {
    ...
    if(!currentUser || currentUser.uid !== "tULsJxwXKQcxf8L4nnmK2oRLViD2") {
      ...
      checkWeek1AttendanceStatus("diego")
    }
  }, [authIsReady])
  ...
  useEffect(() => {
    ...
    if(diegoStudents?.length === Object.keys(weekOneAttendance).length) {
    ...
    <>
        <div className="flex flex-col w-full bg-[url('/images/main_bg.jpg')] bg-cover bg-center">
            <div className="page-header px-3 md:px-6 h-20 bg-gray-300 flex justify-between items-center col-span-2">
                <Link href="/diego"><button className="dcam-btn-rounded flex items-center"><FiArrowLeft className="inline-block me-1" />Main Page</button></Link>
                <h2 className="me-4 text-center">Your next pay day is: <br /> <span className="font-semibold">{closestPayday && closestPayday}</span></h2>
                <Link href="/diego/hours/week2"><button className="dcam-btn-rounded flex items-center">Week 2<FiArrowRight className="inline-block ms-1" /></button></Link>
            </div>

            {/* week 1 form */}
            <form className="py-10 px-3 md:px-20 lg:px-40 xl:px-56" onSubmit={handleSubmitWeekOne}>
                ...
                <table className="bg-gray-100 w-full border-2 border-gray-200 mb-10">
                    ...
                    <tbody id="week1">
                        {diegoStudents?.map((student, index) => (
                            <StudentItem key={index} student={student} handleAttendance={handleAttendance}/>
                        ))}
                    </tbody>
                </table>
                <textarea rows="4" className="w-full p-2 mb-8 border-2 border-gray-400 bg-gray-200" placeholder="Enter any notes you might have pertaining to the attendance here. This could include things like makeup lessons, teacher meetings, etc. The more detailed information, the better!" ref={weekOneNotesRef}/>
...
export default DiegoHoursWeekOne