...
export const RaulHoursWeekTwo = () => {
  ...
  const handleSubmitWeekTwo = async (e) => {
    ...
    try {
        ...
        setTimeout(() => {
          router.push("/raul/hours/success")
        }, 3000)
    } catch(error) {
  ...
  useEffect(() => {
    ...
    if(raulStudents?.length === Object.keys(weekTwoAttendance).length) {
  ...
  useEffect(() => {
    checkWeek2AttendanceStatus("raul")
    fetchData()
    getWeekTwoPayPeriod()
  }, [closestPayday])
    ...
    <>
        <div className="flex flex-col w-full max-w-[100%] bg-[url('/images/main_bg.jpg')] bg-cover bg-center">
            <div className="page-header px-3 md:px-6 h-20 bg-gray-300 flex justify-between items-center col-span-2">
                <Link href="/raul/hours/week1"><button className="dcam-btn-rounded flex items-center"><FiArrowLeft className="inline-block me-1" />Week 1</button></Link>
...
export default RaulHoursWeekTwo