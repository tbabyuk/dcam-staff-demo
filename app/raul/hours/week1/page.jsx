...
export const RaulHoursWeekOne = () => {
  ...
  const [raulStudents, setRaulStudents] = useState(null)
  ...
  const handleSubmitWeekOne = async (e) => {
    ...
    Object.keys(weekOneAttendance).forEach((studentName) => {
      ...
      const studentDocRef = doc(db, 'raul-students', studentName.toLowerCase());
      batch.update(studentDocRef, attendanceData);
    });

      const metaDocRef = doc(db, "meta-data", "raul")
      const metaObject = {
        payday: closestPayday,
  ...
  const fetchData = async () => {
      ...
      // fetch Raul student info upon first render
      const studentsColRef = collection(db, "raul-students")

      const studentArray = []
      const snapshot = await getDocs(studentsColRef)
      snapshot.forEach((doc) => studentArray.push(doc.data()))
      setRaulStudents([...studentArray])
  }


  useEffect(() => {

    if(raulStudents?.length === Object.keys(weekOneAttendance).length) {
  ...
  useEffect(() => {
    checkWeek1AttendanceStatus("raul")
    fetchData()
    getWeekOnePayPeriod()
  }, [closestPayday])
    ...
    <>
        <div className="flex flex-col w-full max-w-[100%]">
            <div className="page-header px-3 md:px-6 h-20 bg-gray-300 flex justify-between items-center col-span-2">
                <Link href="/raul"><button className="dcam-btn-rounded flex items-center"><FiArrowLeft className="inline-block me-1" />Main Page</button></Link>
                <h2 className="me-4 text-center">Your next pay day is: <br /> <span className="font-semibold">{closestPayday && closestPayday}</span></h2>
                <Link href="/raul/hours/week2"><button className="dcam-btn-rounded flex items-center">Week 2<FiArrowRight className="inline-block ms-1" /></button></Link>
            </div>
            ...
            <form className="py-10 px-3 md:px-20 lg:px-40 xl:px-56 border-r-2 border-gray-100" onSubmit={handleSubmitWeekOne}>
                ...
                <table className="bg-gray-100 w-full border-2 border-gray-200 mb-10">
                    ...
                    <tbody id="week1">
                        {raulStudents?.map((student, index) => (
...
export default RaulHoursWeekOne